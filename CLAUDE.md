...
The **11 generator endpoints** on `DocumentController` (all `POST /api/documents/<type>`, returning raw bytes + `X-Generation-Channel` header):

`receipt-order` · `inventory-report` · `revaluation-act` · `write-off-act` · `waybill` (with `?layout=horizontal|vertical` synonymous via Python) · `picking-list` · `placement-list` (after-receipt placement recommendations) · `receipt-act` · `invoice` · `transport-note` (`?layout=horizontal|vertical`) · `cmr`.
