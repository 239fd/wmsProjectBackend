...
- `product-service/rpa/PythonRpaExtractor` — `@Component("oneCExtractor")` that implements `PlannedDeliveryExtractor`. Selected by `ErpExtractorJob` when `erp.extraction.mode=onec`. Calls `POST /parse/supplies` and flattens the hierarchical JSON into one row per supply-item (`{externalId=<supply.external_id>#<sku|row>, supplierName, productName, expectedQuantity, expectedDate}`); `externalId` is a UNIQUE column in `planned_deliveries` so the per-row composite key dedupes across reruns.
...
### Channel 2 — Apache POI (Java, in-process, default)

`document-service/rpa/DocumentRpaService` fills `.xls` / `.docx` templates from `document-service/documents template/` (Cyrillic filenames — keep them, the service references them by exact name) using Apache POI; `PdfDocumentService` then converts to PDF (`defaultValue = "pdf"` on the `format` query param). XLS/DOCX is still available via `?format=xls`/`?format=docx`. This is the **primary** channel — selected when `X-Generation-Mode` is absent or `auto`. Works on every OS (Linux Docker, Windows, k8s). The 14 generators on `DocumentController` (all `POST /api/documents/<type>`):
