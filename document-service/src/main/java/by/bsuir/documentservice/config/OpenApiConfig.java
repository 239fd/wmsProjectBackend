    ...
    public OpenAPI documentServiceAPI() {
        ...
        Info info =
                new Info()
                        ...
                        .version("1.0.0")
                        .description(
                                "REST API для генерации документов в системе WMS с использованием RPA (Robotic Process Automation). "
                                        + "Поддерживает генерацию накладных, актов переоценки, инвентаризационных описей и других складских документов.")
                        .license(license);

        return new OpenAPI().info(info).servers(List.of(localServer, gatewayServer));
    }
}
