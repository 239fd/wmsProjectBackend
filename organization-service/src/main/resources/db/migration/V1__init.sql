...
CREATE TABLE IF NOT EXISTS organization_read_model (
    org_id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name            VARCHAR(255) NOT NULL,
    short_name      VARCHAR(255),
    unp             VARCHAR(512) NOT NULL,
    address         VARCHAR(512),
...
CREATE INDEX IF NOT EXISTS idx_organization_employees_user_id ON organization_employees(user_id);
CREATE INDEX IF NOT EXISTS idx_organization_employees_org_id ON organization_employees(org_id);
CREATE INDEX IF NOT EXISTS idx_organization_employees_is_active ON organization_employees(is_active);
...
CREATE INDEX IF NOT EXISTS idx_invitation_codes_org_id ON organization_invitation_codes(org_id);
CREATE INDEX IF NOT EXISTS idx_invitation_codes_code ON organization_invitation_codes(invitation_code) WHERE is_active = TRUE;
...
CREATE TABLE IF NOT EXISTS organization_invitations (
    ...
    used_at             TIMESTAMP,
    used_by             UUID,
    CONSTRAINT fk_invitation_new_org FOREIGN KEY (org_id) REFERENCES organization_read_model(org_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_invitations_token ON organization_invitations(invitation_token);
CREATE INDEX IF NOT EXISTS idx_invitations_org_id ON organization_invitations(org_id);
